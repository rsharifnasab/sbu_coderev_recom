...
class MRLoaderData(Logger):
    ...
    def prepare(self, data):
        ...
        # unify timezone
        data["commits"]["committed_date"] = pd.to_datetime(
            data["commits"].committed_date
        ).dt.tz_localize(None)

        # remove commits not in the timeframe
        data["commits"] = data["commits"][
            time_interval(
                data["commits"]["committed_date"], self.from_date, self.to_date
            )
        ]
        ...
        commits = (
            data["commits"]
            .merge(data["commits_file"], left_on="key_commit", right_on="key_commit")
            .merge(data["commits_author"], left_on="key_commit", right_on="key_commit")
        )

        commits["date"] = pd.to_datetime(
            commits.committed_date).dt.tz_localize(None)
        ...
        commits["key_file"] = commits["key_file"].apply(
            lambda x: x.replace(":", "/"))
        ...
        data["changes"]["created_at"] = pd.to_datetime(
            data["changes"].created_at
        ).dt.tz_localize(None)

        # remove pulls not in the time frame
        data["changes"] = data["changes"][
            time_interval(data["changes"]["created_at"],
        ...
        # data cleaning
        pulls = pulls.drop(["index_x", "index_y", "index"], axis=1)

        pulls["updated_at"] = pd.to_datetime(
            pulls.updated_time).dt.tz_localize(None)
        pulls["created_at"] = pd.to_datetime(
            pulls.created_at).dt.tz_localize(None)

        # reformatting and renaming
        pulls["key_file"] = pulls["key_file"].apply(
            lambda x: x.replace(":", "/"))
        pulls = pulls.rename(
            {
                "key_file": "file_path",
                "subject": "body",
                "key_user_y": "reviewer_login",
                "key": "number",
                "key_user_x": "owner",
            },
            axis=1,
        ...
        pulls["comment"] = pulls.comment.apply(
            lambda x: x.split("Reviewed-on")[0])

        # comments part
        comments_file = data["comments_file"]
        comments_file.time = pd.to_datetime(
            comments_file.time).dt.tz_localize(None)
        comments_file = comments_file[
            time_interval(comments_file.time, self.from_date, self.to_date)
        ]
        comments_file = comments_file.drop(["index"], axis=1).rename(
            {"time": "date"}, axis=1
        )
        comments_file["key_file"] = comments_file["key_file"].apply(
            lambda x: x.replace(":", "/")
        )

        comments_pull = data["comments_patch"]
        comments_pull.time = pd.to_datetime(
            comments_pull.time).dt.tz_localize(None)
        comments_pull = comments_pull[
            time_interval(comments_pull.time, self.from_date, self.to_date)
        ]

        comments_pull = comments_pull.drop(["index", "oid"], axis=1).rename(
            {"time": "date"}, axis=1
        )
        comments_pull["key_file"] = None

        comments = (
            pd.concat((comments_pull, comments_file), axis=0)
            .reset_index()
            .drop(["index"], axis=1)
        )
    ...
    def prepare_pulls(self):
        ...
        self.pulls = (
            self.pulls.groupby("key_change")[
                ["file", "reviewer", "date", "owner", "title", "status", "closed"]
            ]
            .agg(
                {
        ...
        pull_authors = (
            self.commits.groupby("key_change")
            .agg({"key_user": lambda x: set(x)})
            .reset_index()
        )
        pull_authors = pull_authors.rename({"key_user": "author"}, axis=1)
        self.pulls = self.pulls.merge(
            pull_authors, on="key_change", how="left")
    ...
    def from_checkpoint(self, path):
        ...
        self.pulls.owner = self.pulls.owner.apply(
        ...
        try:
            self.pulls.reviewer = self.pulls.reviewer.apply(
        ...
        if os.path.isfile(path + "/commits.csv"):
            ...
            self.commits.date = pd.to_datetime(
                self.commits.date).dt.tz_localize(None)
        else:
        ...
        if os.path.isfile(path + "/comments.csv"):
            ...
            # , index_col=0)
            self.comments.date = pd.to_datetime(
                self.comments.date).dt.tz_localize(None)
            print(f"loaded {self.comments=}")
        else:
            self.commits = pd.DataFrame(
                columns=["key_user", "key_change", "date", "key_file"]
            )
