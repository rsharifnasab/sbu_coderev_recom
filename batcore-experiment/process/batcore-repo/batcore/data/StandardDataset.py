...
from batcore.data.utils import ItemMap, add_self_review, preprocess_users
...
class StandardDataset(DatasetBase, Logger):
    ...
    def get_pulls(self, dataset):
        ...
        filtered_pulls = pulls[pulls["file"].apply(len) > self.max_file]

        # Check if the filtered DataFrame is empty
        if not filtered_pulls.empty:
            self.bad_pulls = set(filtered_pulls["key_change"])
        else:
            self.bad_pulls = set()

        if self.remove_empty:
            # remember pull w/out reviewers
            self.bad_pulls = self.bad_pulls.union(
                set(pulls[pulls.reviewer.apply(len) == 0]["key_change"])
            )
        ...
        if self.owner_policy == "author":
            ...
            pulls.owner = pulls.apply(
                lambda x: list(x.author) if len(x.author) else (x.owner), axis=1
            )
        ...
        if len(self.remove):
            for col in self.remove:
                pulls.reviewer = pulls.apply(
                    lambda x: [rev for rev in x["reviewer"]
                               if rev not in x[col]],
                    axis=1,
                )
    ...
    def get_comments(self, dataset):
        ...
        print(comments.head())
        print(comments["key_change"].head())
    ...
    def from_checkpoint(self, path):
        ...
        self.data["pulls"].date = pd.to_datetime(
            self.data["pulls"].date
        ).dt.tz_localize(None)

        self.data["pulls"].file = self.data["pulls"].file.apply(
            ast.literal_eval)
        self.data["pulls"].reviewer = self.data["pulls"].reviewer.apply(
            ast.literal_eval
        )
        ...
        if self.commits:
            self.data["commits"] = pd.read_csv(
                path + "/commits.csv", index_col=0)
            self.data["commits"].date = pd.to_datetime(
                self.data["commits"].date
            ).dt.tz_localize(None)

        if self.comments:
            self.data["comments"] = pd.read_csv(
                path + "/comments.csv", index_col=0)
            self.data["comments"].date = pd.to_datetime(
                self.data["comments"].date
            ).dt.tz_localize(None)
