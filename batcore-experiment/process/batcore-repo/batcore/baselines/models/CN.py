...
class CN(BanRecommenderBase):
    ...
    def __init__(
        self, items2ids, lambd=0.5, no_owner=True, no_inactive=True, inactive_time=60
    ):
        ...
        for event in data:
            ...
            if event["type"] == "pull":
                ...
                if len(event["reviewer"]):
                    rev_ind = np.array(
                        [self.users.getid(rev) for rev in event["reviewer"]]
                    )
                ...
                val = (
                    pow(self.lambd, self.com_cnt[pull][j])
                    * (date - self.start_time).seconds
                )
    ...
    def predict_community(self, i, k=10):
        ...
        com_id, com_size = np.split(
            np.array(list(com_size.items())), 2, axis=1)
        ...
        for com in com_id:
            com_nodes[com] = sorted(
                com_nodes[com], key=lambda x: -self.in_deg[x])
